...
実装タスク:
- [ ] `SimpleVectorStore` 依存箇所を洗い出し、移行対象一覧を固定する
- [ ] `VectorStoreConfiguration` の戻り値型を `VectorStore` へ寄せる方針を固める
- [ ] `AiConfiguration` の `QuestionAnswerAdvisor` 連携を `VectorStore` / `VectorStoreRetriever` 前提に変更する
- [ ] `BriefingService` の依存を `SimpleVectorStore` から `VectorStoreRetriever` へ変更する
- [ ] `VectorDocumentService` の依存を `SimpleVectorStore` から `VectorStore` へ変更する
- [ ] `SimpleVectorStore.save(...)` 前提の保存処理を抽象化し、ストア固有処理を切り離す
- [ ] `VectorStorePaths` の JSON ファイル前提 API を整理し、SQLite 用パス追加方針を決める
- [ ] 既存ユニットテストを `VectorStore` 抽象前提へ更新する
- [ ] 抽象化リファクタの回帰テストを追加する
...
実装タスク:
- [ ] SQLite ベースの `VectorStore` 実装クラスを追加する
- [ ] 初期実装は `documents` と `document_chunks` の 2 テーブル構成にする
- [ ] `documents` テーブルのスキーマを定義する
- [ ] `document_chunks` テーブルのスキーマを定義する
- [ ] 検索結果の返却単位を chunk 単位にすることを明記する
- [ ] `documents.doc_id` と `document_chunks.chunk_id` の一意制約を定義する
- [ ] `documents` / `document_chunks` の外部キー制約と cascade delete 方針を定義する
- [ ] `docId` `source` `chunkId` 向け index を追加する
- [ ] 文書本文、metadata、embedding を保存する insert 処理を実装する
- [ ] chunk ID 指定で削除する delete 処理を実装する
- [ ] `docId` 指定で削除する delete 処理を実装する
- [ ] `source` 指定で削除する delete 処理を実装する
- [ ] 再登録時の置換方針を `source` 単位の delete + insert で統一する
- [ ] `SearchRequest` を受けて候補 chunk を取得する read 処理を実装する
- [ ] `topK` と `similarityThreshold` を適用する処理を追加する
- [ ] metadata による `source` / `docId` 絞り込みを適用する処理を追加する
- [ ] 類似度指標を cosine similarity に固定する
- [ ] ベクトル正規化の実施タイミングを決める
- [ ] embedding 次元不一致時のエラー方針を決める
- [ ] 類似度計算を Java 側で行う暫定実装を追加する
- [ ] 埋め込みベクトルの保存形式を JSON か BLOB かで決めて統一する
- [ ] 埋め込みベクトル保存形式の判断基準として、実装容易性、読み出し性能、サイズ効率、デバッグ容易性を明記する
- [ ] 文書登録を 1 文書 1 トランザクションで実行する
- [ ] 文書削除を関連行込みで 1 トランザクションで実行する
- [ ] 部分失敗時のロールバック方針を定義する
- [ ] SQLite 初期化処理を `DataSourceConfiguration` と整合する形で追加する
- [ ] SQLite ファイル破損時の扱いを定義する
- [ ] SQLite ロック発生時の扱いを定義する
- [ ] metadata 欠損データの扱いを定義する
- [ ] 存在しない chunk / doc 削除時の扱いを定義する
- [ ] スキーマ初期化のユニットテストを追加する
- [ ] 複数文書・複数 chunk 登録テストを追加する
- [ ] `docId` / `source` 絞り込み検索テストを追加する
- [ ] `topK` と `similarityThreshold` の境界値テストを追加する
- [ ] cascade delete / 関連削除テストを追加する
- [ ] 再登録時の置換テストを追加する
- [ ] 空ストア検索テストを追加する
- [ ] embedding 次元不一致テストを追加する
...
実装タスク:
- [ ] `VectorDocumentService` の保存先を SQLite ベクトルストアへ切り替える
- [ ] `docId` / `source` / `chunkIndex` / `ingestedAt` を SQLite 側で保持するようにする
- [ ] `embed add` の再登録時に source 単位で置換できることを確認する
- [ ] `embed search` の結果整形が載せ替え後も維持されるようにする
- [ ] `embed list` の source 集約表示が載せ替え後も維持されるようにする
- [ ] `embed delete --doc-id` が載せ替え後も維持されるようにする
- [ ] `embed delete --source` が載せ替え後も維持されるようにする
- [ ] AI ツール `vectorDocumentAdd` `vectorDocumentSearch` `vectorDocumentList` `vectorDocumentDeleteByDocId` `vectorDocumentDeleteBySource` の回帰テストを追加する
- [ ] `QuestionAnswerAdvisor` の検索結果が載せ替え後も使えることを確認する
- [ ] `dailyBriefing` の関連文書取得が載せ替え後も使えることを確認する
...
実装タスク:
- [ ] `vector-store.json` 前提の保存・読込コードを削除する
- [ ] `vector-documents.json` 前提の保存・読込コードを削除する
- [ ] `VectorStorePaths` の JSON 用パス API を削除または SQLite 用へ置き換える
- [ ] README から JSON ベクトルストア前提の説明を削除する
- [ ] 旧 JSON 永続化コード削除後の回帰テストを追加する
...
- [ ] README に SQLite ベクトルストアの構成と制約を追記する
- [ ] 類似度計算を Java 側で行う場合の性能制約を README に明記する
