...
- [ ] 0.1 `pom.xml` に Web / Security / Actuator 依存を追加する
...
- [ ] 1.1 `ApiKeyProperties` を追加する
  ...
  - Green: `ApiKeyProperties` と登録処理を実装し、起動判定と認証に使うキーの source of truth を `REI_API_KEY` に統一する。`application.yml` にキーを定義しない。
  - Refactor: プロパティ名と環境変数 `REI_API_KEY` の対応を整理する。
  - _Requirements: 2.4, 2.9_

- [ ] 1.2 `WebApplication` の bootstrapping（`WebApplicationType` 判定）を実装する
...
- [ ] 1.3 `ReiApplication.main()` に bootstrapping を組み込む
...
- [ ] 2.1 `ApiKeyAuthenticationFilter` を実装する
...
- [ ] 3.2 `RunRegistry` を実装する（状態遷移）
  ...
  - Green: `RunRegistry` を実装する（`compareAndSet` による atomic transition）。
  - Refactor: 状態遷移の invariant を明示する。
  - _Requirements: 12.1, 12.2, 12.7_

- [ ] 3.3 `RunRegistry` の retention / purge を実装する
...
- [ ] 3.4 `GET /api/v1/runs/{runId}` を実装する
...
- [ ] 4.2 `ProjectRegistry.resolveById()` を追加する
...
- [ ] 4.3 `ChatSubmitService` を実装する（session / project 整合性 + ID 採番）
...
- [ ] 5.1 `ProjectRunQueue` を実装する（enqueue / cancelQueued）
...
- [ ] 5.2 `ConversationInputRouter` の FIFO 責務を `ProjectRunQueue` へ移す
...
- [ ] 5.3 runner のライフサイクルと異常終了回収を接続する
...
- [ ] 5.4 project / session / run の分離を実際の実行経路で検証する
...
- [ ] 6.1 `AgentEventType.AGENT_RUN_CANCELLED` を追加する
...
- [ ] 6.2 `AgentEventFactory.runCancelled()` を追加する
...
- [ ] 6.3 `CommandCancellationService.cancelRun(runId)` を追加する
...
- [ ] 6.5 `RunController` に cancel エンドポイントを追加する
...
- [ ] 6.6 queue / runner を接続した cancel の競合を検証する
...
- [ ] 7.2 `SseBridge` を実装する（bounded queue + executor）
...
- [ ] 8.2 `AgentEventBus.subscribe(fromSequence)` を拡張する
...
- [ ] 8.3 SSE の replay とライブ購読の競合防止を実装する
...
- [ ] 8.4 `Last-Event-ID` の replay と replay gap 判定を実装する
...
- [ ] 8.5 terminal 状態と空 replay の競合を実装する
