...
## 追補設計 (2026-05-08): 回答ヘッダの経過時間表示

### 目的
- 回答ヘッダ固定文字列 `=== answer ===` を、経過時間付き `=== answer(<seconds> s) ===` に更新する。

### 設計方針
1. `ChatCommand` / `SearchCommand` の `run()` 冒頭で開始時刻（`System.nanoTime()`）を取得する。
2. 回答ヘッダ出力直前に `System.nanoTime() - startedAt` から経過秒を算出する。
3. 表示フォーマットは `Locale.ROOT` + `"%.1f"` で小数第1位固定にする。
4. ヘッダ文字列組み立ては専用メソッド（例: `answerHeader(long startedAtNanos)`）で共通パターン化する。
...
### 非影響要件
- ストリーミング本文の出力順序は変更しない。
- タイムアウト/キャンセル/エラー時の分岐は変更しない。
- `chat-response-narration` による読み上げ判定ロジックには影響を与えない。

## 追補訂正 (2026-05-08)

追補設計の手順2と非影響要件を次のように読み替える。
- 手順2: 経過秒は「最初の回答チャンク受信時」に算出し、そのタイミングでヘッダを初回出力する。
- 非影響要件: ヘッダの出力タイミング変更は本文ストリーミングの先頭出力と同時であり、本文内容・タイムアウト・キャンセル・読み上げ制御には影響しない。
