...
| `sanitize(String text)` | `text`: 入力テキスト | `String` | Markdown コードブロック・絵文字・制御文字・非表示文字を除去したテキストを返す（`static`） |
...
### プロパティ 4: サニタイズでコードブロック・絵文字・制御文字が除去される

*任意の* テキストに対して、`sanitize()` を呼び出したとき、結果に Markdown コードブロック（` ``` ` で囲まれたブロック）・絵文字（Unicode So カテゴリ）・サロゲートペア（Cs カテゴリ）・未割当文字（Cn カテゴリ）・制御文字（タブ・改行・復帰を除く）が含まれない。タブ・改行・復帰およびインラインコード（バッククォート1つ）は保持される。

**Validates: 要件 5.2, 5.3, 5.4, 5.5, 5.6, 5.7, 5.8**
