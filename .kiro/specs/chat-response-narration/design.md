...
| `sanitize(String text)` | `text`: 入力テキスト | `String` | Markdown 記法（コードブロック・見出し・太字・斜体・リンク・引用・表・水平線）・絵文字・制御文字を除去し、箇条書き `*` を `-` に変換したテキストを返す（`static`） |
...
*任意の* テキストに対して、`sanitize()` を呼び出したとき、結果に Markdown コードブロック・見出し記号・太字/斜体記号・インラインコードのバッククォート・リンクの URL・引用記号・水平線・表行・絵文字（Unicode So カテゴリ）・サロゲートペア（Cs カテゴリ）・未割当文字（Cn カテゴリ）・制御文字（タブ・改行・復帰を除く）が含まれない。タブ・改行・復帰、箇条書き `-`、および箇条書き `*` を `-` に変換したものは保持される。
