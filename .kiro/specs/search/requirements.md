...
**ユーザーストーリー:** ユーザーとして、検索質問を送ってから回答が始まるまでの待ち時間を把握したいので、回答ヘッダに経過時間を表示してほしい。

#### 受け入れ基準
1. WHEN `search` コマンドで回答表示が開始されるとき、THE SearchCommand SHALL 回答ヘッダを `=== answer(<seconds> s) ===` 形式で表示する。
2. THE SearchCommand SHALL `<seconds>` を秒単位・小数第1位で表示する。
3. THE SearchCommand SHALL 待ち時間の起点を「検索コマンド実行開始」、終点を「回答ヘッダ出力時刻」とする。
4. THE SearchCommand SHALL 既存のソース表示・Web検索スキップ表示・タイムアウト・キャンセル挙動を変更しない。
