...
@Component
@Command(name = "memory", description = "記憶整理関連コマンド", subcommands = {
...
public class MemoryCommand {
  ...
  @Command(name = "list", description = "保存済み記憶を表示します")
  ...
  public static class ForgetCommand implements Runnable {
    ...
    public void run() {
      if (id == null || id.isBlank()) {
        System.out.println("有効な記憶 ID を指定してください");
        return;
      }
  ...
  public static class ConsolidateCommand implements Runnable {
    ...
    public void run() {
      List<Memory> candidates = consolidatorService.extractCandidates();
      if (candidates.isEmpty()) {
        System.out.println("整理対象の会話履歴がありません");
        return;
      }
