...
public final class ActionValidator {
  ...
  public static void validate(ComputerAction action, CapturedScreen screen) {
    if (action == null || action.risk() == null) throw new IllegalArgumentException("Missing action or risk");
    switch (action) {
      ...
      case ComputerAction.PressKey a -> {
        if (!KEYS.contains(a.key() == null ? "" : a.key())) throw new IllegalArgumentException("Unknown key");
      }
      case ComputerAction.Scroll a -> { if (a.amount() == 0 || Math.abs((long) a.amount()) > 20) throw new IllegalArgumentException("Invalid scroll"); }
      case ComputerAction.Wait a -> { if (a.millis() < 1 || a.millis() > 10000) throw new IllegalArgumentException("Invalid wait"); }
  ...
  static void text(String text, int limit) {
    if (text == null || text.isBlank() || text.length() > limit) throw new IllegalArgumentException("Missing or oversized text");
  }

  private static void target(ComputerAction.Target target, double confidence, CapturedScreen screen) {
    if (target == null) throw new IllegalArgumentException("Missing target");
    ...
    if (target.x() < 0 || target.y() < 0 || target.x() >= screen.image().getWidth() || target.y() >= screen.image().getHeight())
      throw new IllegalArgumentException("Target outside screenshot");
  }
}
