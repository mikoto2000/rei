...
public record SemanticSessionPolicy(Duration normalGap,Duration maximumGap,Duration briefSwitch,double minimumConfidence,ZoneId zone) {
  public SemanticSessionPolicy(Duration maximumGap,Duration briefSwitch,ZoneId zone) {
    this(maximumGap.compareTo(Duration.ofSeconds(120))<0?maximumGap:Duration.ofSeconds(120),maximumGap,briefSwitch,.5,zone);
  }
  public SemanticSessionPolicy {
    new SessionGapPolicy(normalGap,maximumGap);new ActivityRolePolicy(minimumConfidence);
    if(briefSwitch.isNegative()) throw new IllegalArgumentException("Negative summary threshold");
  }
  ...
  public List<SummarySegment> aggregate(List<ActivityRecord> records,Instant from,Instant until) {
    ...
    var groups=new ArrayList<List<Sample>>();var rolePolicy=new ActivityRolePolicy(minimumConfidence);
    for(int i=0;i<sorted.size();i++) {
      ...
      if(!groups.isEmpty() && adjacent(groups.getLast().getLast(),sample) && compatible(groups.getLast(),sample)
          && gapAllows(groups.getLast(),List.of(sample))) groups.getLast().add(sample);
    ...
    for(int i=0;i+2<groups.size();) {
      ...
      if(Duration.between(b.getFirst().start(),b.getLast().end()).compareTo(briefSwitch)<=0
          ...
          && gapAllows(a,b) && gapAllows(join(a,b),c) && projectsCompatible(join(a,b),c)
  ...
  private static List<Sample> join(List<Sample> a,List<Sample> b) {var result=new ArrayList<>(a);result.addAll(b);return result;}
  private boolean projectsCompatible(List<Sample> a,List<Sample> b) {
    return join(a,b).stream().map(s->s.roles().primary()).filter(Objects::nonNull).map(Activity::projectCandidate)
        .filter(p->!p.isBlank()).distinct().count()<=1;
  }
  private boolean gapAllows(List<Sample> a,List<Sample> b) {
    var all=join(a,b);long observed=all.stream().mapToLong(Sample::seconds).sum();
    long missing=Duration.between(all.getFirst().start(),all.getLast().end()).getSeconds()-observed;
    return new SessionGapPolicy(normalGap,maximumGap).allows(Duration.between(a.getLast().end(),b.getFirst().start()),missing,
        strongAgreement(a.getLast().roles(),b.getFirst().roles()));
  }
  public boolean strongAgreement(ActivityRoles a,ActivityRoles b) {
    var x=a.primary();var y=b.primary();
    if(x==null || y==null || !ActivityRolePolicy.sameContext(x,y)) return false;
    return (!x.projectCandidate().isBlank() && x.projectCandidate().equals(y.projectCandidate()))
        || (!x.service().isBlank() && x.service().equals(y.service()) && x.application().equals(y.application()));
  }
  ...
  private SummarySegment segment(List<Sample> samples) {
    ...
    long seconds=samples.stream().mapToLong(Sample::seconds).sum(); double weighted=0;long supporting=0;
    for(var s:samples) {
      if(primary!=null && ActivityRolePolicy.sameRole(primary,s.roles().primary())) {weighted+=s.seconds()*s.roles().confidence();supporting+=s.seconds();}
      for(var a:s.record().inference().activities().stream().map(ActivityVocabulary::canonical).toList()) {
    ...
    double confidence=supporting==0?0:weighted/supporting;
    for(int i=1;i<samples.size();i++) if(Duration.between(samples.get(i-1).end(),samples.get(i).start()).compareTo(normalGap)>0) {confidence*=.85;break;}
    if(confidence<minimumConfidence || supporting*2<seconds) {if(primary!=null) secondary.add(primary);primary=null;confidence=0;}
    var roles=new ActivityRoles(primary,List.copyOf(secondary),List.copyOf(background),confidence,
    ...
    return new SummarySegment(samples.getFirst().start(),samples.getLast().end(),seconds,roles,
        samples.stream().map(Sample::record).toList(),List.of(),SummaryGroupingPolicy.theme(roles),
        samples.stream().map(s->s.roles().category()).filter(c->!c.equals("unknown")).distinct().toList());
  }
}
