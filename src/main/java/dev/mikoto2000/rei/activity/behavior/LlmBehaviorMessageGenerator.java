...
public final class LlmBehaviorMessageGenerator implements BehaviorMessageGenerator {
  ...
  @Override public String generate(BehaviorNotification notification) throws Exception {
    ...
    String instruction="""
        ...
        NOTICEは明確に注意し、「そろそろ区切りをつけて」と行動を促してください。
        WARNINGは強めに注意し、「ここでいったんやめて」と中断を求めてください。
        STRONG_WARNINGは厳しく注意し、「今すぐやめて、画面から離れて」と即時の中断を求めてください。
        上記は口調の例です。「〜しようか」「よければ」などの遠慮した提案で注意を弱めず、Severityごとの強さを保ってください。
        世話焼きではっきりした小言にしますが、注意は行動に向け、侮辱・羞恥・人格評価をしないでください。
