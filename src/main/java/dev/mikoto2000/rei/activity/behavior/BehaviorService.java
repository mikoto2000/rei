    ...
    try {
      ...
      synchronized(this) {
        if(!allowed(token) || tracker.isAgentBusy() || tracker.activityVersion()!=activityVersion
            ...
            || current.recoveredAt()!=null && (a.recoveredAt()==null || current.recoveredAt().isAfter(a.recoveredAt()))) {suppression="CONTEXT_CHANGED";return;}
        if(message==null || message.isBlank()) throw new IllegalStateException("Empty behavior message");
        publisher.publish(new AgentMessage(UUID.randomUUID().toString(),"assistant",message,MessageOrigin.BEHAVIOR,clock.instant()));
        suppression="DELIVERED";
      }
    } catch(Exception error) {suppression="FAILED";log.warn("Behavior evaluation/notification failed ({})",error.getClass().getSimpleName());}
