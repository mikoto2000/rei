...
class ContextCompletionTest {
  ...
  @Test void activityActionsCompleteWithoutExecutingCaptureOrTimeline() {
    var cmd = command();
    assertThat(values(cmd, "/activity ")).containsExactlyInAnyOrder("today", "yesterday", "summary", "pause", "resume", "behavior");
  ...
  @Test void behaviorActionsCompleteFromSubcommandDefinition() {
    assertThat(values(command(), "/activity behavior ")).containsExactlyInAnyOrder("on","off","status","evaluate");
