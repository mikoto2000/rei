...
2. 再推定後の座標を元画面に赤いリングで示し、その中心が対象のクリック可能な領域内か確認する。

両方で厳密な JSON の `approved: true` と空でない理由が返った場合だけクリックへ進みます。拒否・不正応答・出力打ち切り・通信例外は MODEL_ERROR で停止します。キャンセルはキャンセルとして扱います。検証では新しい座標や操作は受け付けません。通常のモデル呼び出しは判断1回、位置特定2回、検証2回です。モデルの誤承認までは防げないため、位置精度は実環境での確認が必要です。

診断有効時は `crop-verification-input.png` / `request.txt` / `response.txt` および `point-verification-input.png` / `request.txt` / `response.txt`（各ファイル名に段階の接頭辞が付きます）を保存します。通信例外は各段階の `error.txt` に記録します。最後の画像は元画面の縮小コピーに目印を描いたもので、元画像や画面自体は変更しません。
