...
`ThemeActivityFormatter` は選定済み activity の順序を保って日本語へ変換する。
1件なら `rei の開発`、複数なら `音声入力・文字起こし系に関する文書作業や開発`、
3件なら `rei に関する文書作業や開発、調査` とする。テーマ名そのものの `・` は変更しない。
集計値、ランキングの重み、alias/group の条件、activity の選定件数は変更しない。

`DailySummaryBucketFormatter` は日次の `leisureActivities` に含まれる SNS・動画/音楽・
ショッピング・ゲームが複数時間帯に現れる場合、補助的な反復を時間帯別から省く。
既存のカテゴリ別秒数と secondary/background の意味ラベルを使い、観測秒数の65%以上を占める
最上位活動、作業テーマがない時間帯の先頭の既知活動、時間帯固有の活動は残す。
日次の非作業活動一覧と娯楽判定は変更しない。
LLM 成功時も対象時間帯はこの formatter で再描画する。対象外の時間帯の LLM 文は維持する。
追加の LLM 呼び出しやプロンプト変更はない。

時間は `画面観測データあり: … / 観測データなし: …` と表示し、
PC利用時間や作業時間ではないことを明記する。秒数の計算と日付範囲は従来どおり。
