...
既定の Web 検索プロバイダは DuckDuckGo です。API キーなしで使えます。

```bash
export REI_WEB_SEARCH_ENABLED=true
export REI_WEB_SEARCH_PROVIDER=duckduckgo
```

Brave Search API を使いたい場合は、`provider` を切り替えて API キーを設定してください。

```bash
export REI_WEB_SEARCH_PROVIDER=brave
...
```bash
export REI_WEB_SEARCH_BASE_URL=
...
| `REI_WEB_SEARCH_PROVIDER` | 任意 | `duckduckgo` | `duckduckgo` または `brave` |
| `REI_WEB_SEARCH_API_KEY` | `provider=brave` の場合必須 | 空 | Brave Search API キー |
| `REI_WEB_SEARCH_BASE_URL` | 任意 | 空 | プロバイダ別の既定 URL を上書き |
...
- `Web search API key is not configured. Set REI_WEB_SEARCH_API_KEY.`
  - `REI_WEB_SEARCH_PROVIDER=brave` の場合は `REI_WEB_SEARCH_API_KEY` を設定してください。未設定でも `/search` はベクトルストアのみで続行します。
