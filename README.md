...
両モードではさらに `computer-use-planner` の判断モデルで2回検証します。再推定前に切り出し画像内で対象を識別できることを確認し、再推定後に元の画面へ赤い目印を描いて、その中心が対象上にあることを確認します。拒否・曖昧な応答・解析エラーでは `MODEL_ERROR` で停止し、推測座標ではクリックしません。1クリックにつき通常は判断1回・位置特定2回・検証2回の計5回のモデル呼び出しになります。モデルによる検証なので、誤認を完全に防ぐ保証はありません。

検証の診断ファイルは `crop-verification-*` と `point-verification-*`（`input.png` / `request.txt` / `response.txt` / 通信例外時 `error.txt`）です。赤い目印は検証用コピーだけに描画し、実際のデスクトップや位置特定用画像は変更しません。
