...
`/project cd` は未登録のフォルダーも登録し、そのプロジェクトで最後に更新されたセッションに切り替えます。セッションがなければ次の入力で新規作成します。Linux / macOS では `/home/your-name/work/my-project` などのパスを指定します。別のプロジェクトに切り替えると、先の処理を続けたまま新しい依頼を開始できます。
